...
<div class="bg-white p-4 rounded-md" style="box-shadow: 4px 4px 10px rgba(0, 0, 0, 0.2);">
    ...
    <div class="flex justify-between items-center mb-4">
        <a href="{{ route('manager.users') }}" class="inline-flex items-center text-black hover:text-blue-800">
            <i class="fas fa-arrow-left mr-2"></i> 
        </a>

        @if(!$editMode)
            <a href="{{ route('manager.users.edit', ['id' => $user->id, 'edit' => 'true']) }}"
        ...
        @if($editMode)
            <form id="editUserForm" method="POST" action="{{ route('manager.users.update', $user->id) }}">
                ...
                <div class="flex justify-end space-x-2">
                    <a href="{{ route('manager.users.edit', $user->id) }}" class="px-4 py-2 bg-gray-300 text-gray-800 rounded hover:bg-gray-400">Cancel</a>
